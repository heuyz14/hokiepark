 ...
 * (supabase/optional/schedule_simulator.sql; every minute by default, every 5 or 10 minutes also fine), so a healthy
 * feed is never much older than its interval. 12 minutes tolerates the slowest documented cadence missing a tick,
 * plus slack. Raise it if the schedule is ever made slower than every 10 minutes.
 */
