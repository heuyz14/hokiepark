...
## Databricks Tier B scaffold (added 2026-09-19)
- `databricks/`: notebooks 01 bronze -> 02 silver -> 03 gold -> 04 MLflow, `databricks.yml` (serverless job), `src/hokiepark_demand.py` (pure-Python port of `src/lib/demand.ts`), `data/` (generated inputs), `README.md` (setup + what is/isn't verified). New scripts: `npm run databricks:inputs`, `npm run test:py`.
- Design forced by Databricks Free Edition (serverless only, restricted outbound internet, volumes not DBFS): the timetable is uploaded to a UC volume, not scraped; the notebook exports `curves.seed.sql` to a volume for the user to run in Supabase (no direct Supabase call).
- Verified locally: 9 Python tests incl. byte-exact parity with `supabase/curves.seed.sql`; notebook 04 executed against a fake spark with real MLflow 3.16 (9 runs, model registered, predict = 95 for Perry L1 Wed 10:30); YAML structure. Bug found+fixed by that run: MLflow 3.x failed to infer the signature (predict got one column) -> explicit signature.
...
## Plan ahead recommender (built 2026-09-19, evening)
- **What:** "Plan" tab (building + weekday + class time + permit -> top 3 places with walk time, forecast fullness at arrival = class start - 15 min, and Likely open / Filling up / Risky), plus Ask understands "2pm class in Hancock Hall" (rule-based time/day parsing, `src/lib/planask.ts`; no LLM). Code: `src/lib/planahead.ts` (pure), `src/lib/planask.ts`, `src/ui/plan.ts`; wired in `src/main.ts` via `withPlanAhead(...)` (assistant.ts untouched).
- **Data:** `src/data/predictions.json` = the REAL workspace output of notebooks 05-07 (model `workspace.hokiepark.hokiepark_occupancy_forecaster` v1, 94 places x 5 weekdays x 96 buckets, 131 KB). Validated: schema OK, matches units.json ids/capacities, and agrees with the local run to 0.15 pts mean. To refresh: rerun 05-07, download `out/predictions.json`, replace the file, `npm test`.
- **Safety rules (tested, mutation-checked):** only permit-rule "yes" places are RECOMMENDED; "check" places are listed separately, max 2, within 800 m, ordered by distance with no forecast label; "no" places are never shown; a garage is scored only on levels the permit covers (plain C/G never gets Perry; the Perry permit only gets its commuter level); lots with no known class (66 of 85) are never recommended. No permit chosen -> asks for one.
