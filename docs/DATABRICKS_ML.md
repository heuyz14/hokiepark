...
| Timetable building code -> campus building | **Real** | VT's official code list (`hzskvtsc.P_DispBldgList`) matched to VT GIS building numbers; 71 codes, 99.1% of weekly seats placed; manual overrides recorded in a `how` column | `data/timetable-building-codes.json` -> Delta `silver_building_dim` |
