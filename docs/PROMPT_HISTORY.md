...
## 21. 2026-09-20 - deploy, cron, origin lock, disk, runbook refresh, and a bug report
...
## 23. 2026-09-20 - garage pins v2, one-minute refresh, shorter chip

> make it like before fix it and also it looks a little off center and make sure scaling looks good when zooming in and out / also lets change refreshes to every minute / also consider removing the refresh 15 secs text to preserve space

**Outcome:** Reverted the always-drawn maroon footprints (footprints show only with a permit or a selection, as before). The off-centre look was structural: a garage marker's box held both the P pill and the wheelchair pill, so the pin's centre sat between them; the ADA badge now hangs off the pill's right edge, so pill centre = marker centre = footprint centre (verified in a real browser: identical pixels). Markers scale with zoom (`markerScale`: 0.68 zoomed out, ~1.0 at zoom 16, 1.1 max) via an inner wrapper, because MapLibre owns the marker element's own transform. Live polling default is now 60 s (`DEFAULT_POLL_MS`), matching the server-side tick. The header chip no longer shows the cadence ("Live · 12s ago", "Offline", "Connecting", "Sample data"); the cadence stays in its hover text. **Process slip:** my earlier commit 6b299f0 used `git add src` and swept another session's unfinished advisor work into `main`; the derived advisor bundle was regenerated to bring `main` back to green. Commits now stage named files only.
