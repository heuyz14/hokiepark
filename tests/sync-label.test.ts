...
import { describeSync, relativeAge, STALE_DATA_MS } from "../src/lib/sync-label.ts";
...
test("the other states are unchanged: demo, connecting, offline (with and without a previous sync)", () => {
  assert.deepEqual(describeSync({ state: "demo", lastSync: null, dataAsOf: null }, NOW), { text: "Sample data", title: "Showing bundled sample counts (live feed not configured).", tone: "off" });
  assert.equal(describeSync({ state: "connecting", lastSync: null, dataAsOf: null }, NOW).text, "Connecting");
  const never = describeSync({ state: "offline", lastSync: null, dataAsOf: null }, NOW);
  const had = describeSync({ state: "offline", lastSync: NOW - 60_000, dataAsOf: NOW - 60_000 }, NOW);
...
test("a live status with no data timestamp yet shows Live (nothing to be stale about)", () => {
  assert.equal(describeSync({ state: "live", lastSync: NOW - 2_000, dataAsOf: null }, NOW).tone, "live");
});
